...
def getQRcode():
    ...
    qrData = client.get(getLoginQRApi).json()
...
def qrWaiting(qrcodeKey: str):
    ...
    while True:
        r = client.get(url=waitApi, params=params)
        if url := r.json()["data"]["url"]:
            client.get(url)
            break
    bili_jct = client.cookies.get("bili_jct")
    return bili_jct


def completionCookie(jct: str):
    """
    ...
    apis = client.get(ssoSetApi).json()["data"]["sso"]
    for url in apis:
        client.post(url)